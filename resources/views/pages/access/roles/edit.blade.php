<x-app-layout title="{{ trans('roles.title.edit') }}" is-header-blur="true">
    <main class="main-content w-full px-[var(--margin-x)] pb-8">
        <div class="flex items-center space-x-4 py-5 lg:py-6">
            <h2 class="text-xl font-medium text-slate-800 lg:text-2xl">
                 {{ trans('roles.title.sub_edit') }}
            </h2>
            ...
            <ul class="hidden flex-wrap items-center space-x-2 sm:flex">
                ...
                <li> {{ trans('roles.title.sub_edit') }}</li>
            </ul>
        </div>
            ...
            <div class="card pb-4">
                ...
                <div class="mx-5">
                    <label class="block mt-3 max-w-xl" for="name">
                        ...
                        <input
                          class="@error('name') is-invalid @enderror form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary"
                          type="text" width="20px" placeholder="{{ trans('roles.form_control.input.name.label') }}"
                          id="name" value="{{ old('name', $role->name) }}" name="name"
                          />
                        @error('name')
                            <span class="text-tiny+ text-error"><strong>{{ $message }}</strong></span>
                        @enderror
                    </label>
                                                ...
                                                @if (old('permissions', $permissionChecked))
                                                    <td class="whitespace-nowrap sm:px-5">
                                                        <label class="inline-flex items-center space-x-2" for="permission-{{ $permission }}">
                                                            <input {{ in_array($permission, old('permissions', $permissionChecked)) ? 'checked' : null }} value="{{ $permission }}" name="permissions[]" id="permission-{{ $permission }}" type="checkbox" class="form-checkbox is-basic h-5 w-5 rounded-full border-slate-400/70 checked:!border-success checked:bg-success hover:!border-success focus:!border-success" >
                                                            <span>{{ trans("permissions.{$permission}") }}</span>
                                                        </label>
                                                    </td>
                                                @else
                                                    <td class="border whitespace-nowrap sm:px-5">
                                                        <label class=" inline-flex items-center p-3 space-x-2" >
                                                            <input value="{{ $permission }}" name="permissions[]" id="permission-{{ $permission }}" type="checkbox" class="form-checkbox is-basic h-5 w-5 rounded-full border-slate-400/70 checked:!border-success checked:bg-success hover:!border-success focus:!border-success" >
                                                            <span>{{ trans("permissions.{$permission}") }}</span>
                                                        </label>
                                                    </td>
                                                @endif
                    ...
                    <div class="flex justify-end space-x-2 mt-5">
                        <a href="{{ route('roles.index') }}" class="btn min-w-[7rem] rounded-full border border-slate-300 font-medium text-slate-700 hover:bg-slate-150 focus:bg-slate-150 active:bg-slate-150/80">
                            {{ trans('roles.button.cancel.value') }}
                        </a>
                        <button type="submit" class="btn min-w-[7rem] rounded-full bg-primary font-medium text-white hover:bg-primary-focus focus:bg-primary-focus active:bg-primary-focus/90">
