<x-app-layout title="{{ trans('roles.title.create') }}" is-header-blur="true">
    <main class="main-content w-full px-[var(--margin-x)] pb-8">
        <div class="flex items-center space-x-4 py-5 lg:py-6">
            <h2 class="text-xl font-medium text-slate-800 lg:text-2xl">
                 {{ trans('roles.title.sub_create') }}
            </h2>
            ...
            <ul class="hidden flex-wrap items-center space-x-2 sm:flex">
                ...
                <li> {{ trans('roles.title.sub_create') }}</li>
            </ul>
        </div>
                                            ...
                                            @if (old('permissions'))
                                                <td class="whitespace-nowrap sm:px-5">
                                                    <label class="inline-flex items-center space-x-2" for="permission-{{ $permission }}">
                                                        <input {{ in_array($permission, old('permissions')) ? 'checked' : null }} value="{{ $permission }}" name="permissions[]" id="permission-{{ $permission }}" type="checkbox" class="form-checkbox is-basic h-5 w-5 rounded-full border-slate-400/70 checked:!border-success checked:bg-success hover:!border-success focus:!border-success" >
